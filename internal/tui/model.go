...
type KeyMap struct {
	...
	Mode                                    key.Binding // UI Mode cycling
}
...
var keys = KeyMap{
	...
	Mode:   key.NewBinding(key.WithKeys("m")), // Cycle UI Mode
}
...
type Model struct {
	...
	SignalStatuses map[string]string // Buy outcome per mint: bought, no_sol, max_pos, etc.

	// Phase 5: Track last signal time for TG connection status
...
func NewModel(cfg *config.Manager) Model {
	// Read UI mode from environment (default: 5 = FNEX)
	uiMode := 5
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case SignalMsg:
		...
		// Phase 5: Update last signal time for TG connection status
	...
	case SignalStatusesMsg:
		m.SignalStatuses = msg.Statuses
	case TrackedTokensMsg:
		m.TrackedTokens = msg.Tokens
	}
...
func (m Model) handleGlobalInput(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case ScreenDashboard:
		...
		case key.Matches(msg, keys.Left):
			if m.UIMode == 4 {
				// Mode 4: Left/Right scroll the focused pane (h/l style)
				if m.FocusPane == 1 && m.Signals.Offset > 0 {
					m.Signals.Offset--
				} else if m.FocusPane == 2 && m.Positions.Offset > 0 {
					m.Positions.Offset--
		...
		case key.Matches(msg, keys.Mode):
			// Cycle UI Mode: 1 → 2 → 3 → 4 → 5 → 1
			m.UIMode++
			if m.UIMode > 5 {
				m.UIMode = 1
			}
			// Apply theme for new mode (Neo-Sol Theme System)
			SetThemeByMode(m.UIMode)
			// Reset maps for new mode
			m.UniqueEntries = make(map[string]bool)
			m.Unique2X = make(map[string]bool)
		}
	case ScreenLogs:
...
func (m *Model) adjustConfig(delta float64) {
	m.Config.Update(func(c *config.Config) {
		...
		case 2:
			c.Trading.MaxAllocPercent = maxf(5, c.Trading.MaxAllocPercent+delta*5)
		case 3:
			c.Trading.MaxOpenPositions = maxi(1, c.Trading.MaxOpenPositions+int(delta))
		case 4:
			c.Fees.StaticPriorityFeeSol = maxf(0.0001, c.Fees.StaticPriorityFeeSol+delta*0.001)
		case 5:
...
func (m Model) View() string {
	...
	default:
		...
		default:
			// UIMode: 1=Classic, 2=Crossterm, 3=Animated, 4=Neon, 5=FNEX
			switch m.UIMode {
			case 1:
			...
			case 5:
				return m.renderFnexDashboard()
			default:
				return m.renderFnexDashboard() // Default to FNEX
			}
		}
	}
}
...
// renderAnimatedDashboard - CRT Matrix animated dashboard (UI Mode 3)
func (m Model) renderAnimatedDashboard() string {
	progress := m.Anim.GetStartupProgress()

	// Get theme colors from Neo-Sol Theme System
	theme := GetThemeByMode(m.UIMode)

	// CRT Phosphor palette - build colors array for animation compatibility
	colors := []lipgloss.Color{
		theme.Border, // [0] Primary: Phosphor Green (#33FF33)
		theme.Active, // [1] Secondary: Active highlight
		theme.Text,   // [2] Text: Pale Green (#E0FFE0)
	}
	bgColor := theme.Background // Black (#000000)
	trueBlack := bgColor        // Alias for trueBlack references

	// Animated border using theme primary color
	borderColor := colors[0]

	// During startup animation, show animated logo
	if progress < 1.0 {
		return m.renderStartupAnimation(progress, bgColor, borderColor, colors[0])
	}

	// ═══════════════════════════════════════════════════════════════
	// CRT MATRIX LAYOUT: Retro-Futuristic Terminal Aesthetic
	// ═══════════════════════════════════════════════════════════════

	// Double-line border style (CRT Terminal Look)
...
func SendSignalStatuses(p *tea.Program, statuses map[string]string) {
	p.Send(SignalStatusesMsg{statuses})
}

// SignalStatusesMsg carries buy outcome statuses for TUI display
type SignalStatusesMsg struct {
	Statuses map[string]string // mint -> status (bought, no_sol, max_pos, etc.)
}

// TrackedTokensMsg carries 2X timer tracking data for TUI display
type TrackedTokensMsg struct {
	Tokens map[string]*trading.TrackedToken
}

func SendTrackedTokens(p *tea.Program, tokens map[string]*trading.TrackedToken) {
	p.Send(TrackedTokensMsg{tokens})
}
...
func (m Model) renderNeonDashboard() string {
	...
	leftContent := lipgloss.JoinVertical(lipgloss.Left,
		...
		fmt.Sprintf(" Entries(50%%): %d", entries),
	...
	positionMints := make(map[string]bool)
	for _, pos := range m.Positions.Positions {
		positionMints[pos.Mint] = true
	}
	...
	for i := start; i < len(m.Signals.List) && i < start+visibleSignals; i++ {
		...
		status := "⏳" // Waiting (below threshold)
		color := lipgloss.Color("#555555")

		if s.Type == signalPkg.SignalEntry && s.Value >= m.Config.GetTrading().MinEntryPercent {
			// Get status from SignalStatuses map (set by executor)
			sigStatus := m.SignalStatuses[s.Mint]
			...
			case "already_have":
				...
				color = lipgloss.Color("#ff9900")
			default:
				// Check if in positions (bought but status not set yet)
				if positionMints[s.Mint] {
					status = "💰" // Bought
					color = lipgloss.Color("#00ff00")
				} else {
					status = "🔄" // Processing
		...
		// 2X indicator (overrides others)
		if s.Reached2X {
			status = "🎯" // Hit 2X!
			color = lipgloss.Color("#00ffff")
		}

		// Compact line: HH:MM NAME %% STATUS
		nameLen := 5
		line := fmt.Sprintf("%s %-5s %3.0f%s %s", t, truncate(s.TokenName, nameLen), s.Value, s.Unit, status)
	...
	header := lipgloss.NewStyle().
		...
		Width(w).
		Render(fmt.Sprintf("═══════════  ⚡ AFNEX ⚡  %s LIVE  %s  ═══════════",
			lipgloss.NewStyle().Foreground(lipgloss.Color("#ff0055")).Render(liveIndicator),
...
func (m Model) renderNeonFooter(w int) string {
	...
	modeName := []string{"", "Classic", "Cross", "Matrix", "Neon"}[m.UIMode]

	// Status with mode indicator
	status := fmt.Sprintf(" ⏱ %s │ 💰 %+.2f%% │ 🎨 %s",
		time.Since(m.StartTime).Truncate(time.Second),
		m.Positions.TotalPnLPercent,
		modeName,
	)

	// Controls - compact for fit
	controls := "[M]ode [S]ell [P]ause [E]xport [F9]Clear [Q]uit"
...
// MODE 5: FNEX TRADING BOT v2.1 - CRT Terminal Style (Patched to match mockup)
// ═══════════════════════════════════════════════════════════════════════════════

func (m Model) renderFnexDashboard() string {
	w, h := m.Width, m.Height

	// Responsive minimum widths - use actual terminal size when possible
	if w < 80 {
		w = 80
	}
	if h < 20 {
	...
	// Phosphor CRT palette (exact match to mockup)
	phosphor := lipgloss.Color("#00FF41") // Main bright green
	dimGreen := lipgloss.Color("#008F11") // Dim text/border
	red := lipgloss.Color("#FF3131")      // PnL negative
	darkBg := lipgloss.Color("#000000")   // Black background

	modeName := "MATRIX"

	// ─── SYSTEM STATS (RAM/RPC) from Mode 4 ───
	ramVal := 0
	fmt.Sscanf(m.Header.MemUsage, "%dMB", &ramVal)
	ramPct := (ramVal * 100) / 1024
	if ramPct > 100 {
	...
	// Uptime and USD estimate
	uptime := time.Since(m.StartTime).Truncate(time.Second)
	usdEst := m.WalletBalance * 185.0 // SOL price estimate

	// ─── HEADER - Simple text, no emojis ───
	headerText := lipgloss.NewStyle().
		...
		Width(w).
		Align(lipgloss.Center).
		Render("FNEX TRADING BOT v2.1")

	// Phase 5: Calculate TG connection status
	tgStatus := "TG: OK"
	if !m.LastSignalTime.IsZero() {
		sinceLastSignal := time.Since(m.LastSignalTime)
		if sinceLastSignal > 5*time.Minute {
			tgStatus = fmt.Sprintf("TG: NO SIGNAL (%dm)", int(sinceLastSignal.Minutes()))
		}
	} else {
		tgStatus = "TG: WAITING"
	}

	// ─── STATUS BAR - Enhanced with system info ───
	var statusContent string
	if w < 100 {
		// Compact status for narrow terminals
		statusContent = fmt.Sprintf("● LIVE | %s | %.2f SOL | %s",
			modeName, m.WalletBalance, tgStatus)
	} else {
		statusContent = fmt.Sprintf("● LIVE MODE: %s  //  WALLET: %.2f SOL ($%.0f)  //  RPC: %dms  //  %s  //  UPTIME: %s",
			modeName, m.WalletBalance, usdEst, rpcLatency, tgStatus, uptime)
	}

	// ─── HEADER BLOCK - Title + Status combined ───
	headerBlock := lipgloss.JoinVertical(
		lipgloss.Left,
		headerText,
		lipgloss.NewStyle().Foreground(dimGreen).Background(darkBg).Width(w).Render(strings.Repeat("─", w)),
		lipgloss.NewStyle().Width(w).Align(lipgloss.Center).Foreground(phosphor).Background(darkBg).Render(statusContent),
		lipgloss.NewStyle().Foreground(dimGreen).Background(darkBg).Width(w).Render(strings.Repeat("─", w)),
	)

	// ─── LAYOUT PROPORTIONS - Responsive for narrow terminals ───
	var leftWidth, rightWidth int
	if w < 100 {
		// 55/45 split for narrow terminals
		leftWidth = int(float64(w) * 0.55)
	} else {
		// 62/38 split for wide terminals
		leftWidth = int(float64(w) * 0.62)
	}
	rightWidth = w - leftWidth

	bodyHeight := h - 6 // header block + footer
	if bodyHeight < 10 {
		bodyHeight = 10
	}
	posHeight := bodyHeight / 3           // Small top box
	statsHeight := bodyHeight - posHeight // Larger bottom box
	...
	for i := start; i < len(m.Signals.List) && i < start+visibleSignals; i++ {
		...
		// Get status from SignalStatuses map
		sigStatus := m.SignalStatuses[s.Mint]

		// Truncate token and mint
		tokenName := s.TokenName
		if len(tokenName) > 6 {
			tokenName = tokenName[:6]
		}
		mintShort := s.Mint
		if len(mintShort) > 10 {
			mintShort = mintShort[:7] + "..." + mintShort[len(mintShort)-3:]
		}
		...
		case "already_have":
			...
			lineStyle = lipgloss.NewStyle().Foreground(dimGreen)
		case "rpc_error":
			statusText = "FAIL: RPC ERROR"
			lineStyle = lipgloss.NewStyle().Foreground(red)
		case "duplicate":
			statusText = "SKIP: DUPLICATE"
			lineStyle = lipgloss.NewStyle().Foreground(dimGreen)
		case "monitoring":
			statusText = "MONITORING ONLY"
			lineStyle = lipgloss.NewStyle().Foreground(dimGreen)
		default:
			// Handle retry_N pattern (Phase 4)
			if len(sigStatus) > 6 && sigStatus[:6] == "retry_" {
	...
	signalBox := panelStyle.
		Width(leftWidth - 2).
		Height(bodyHeight - 2).
		Render(signalTitle + "\n" + signalContent)

	// ─── OPEN POSITIONS PANEL ───
	posTitle := lipgloss.NewStyle().
		Foreground(phosphor).
		Bold(true).
		Render("OPEN POSITIONS [AUTO-MANAGE]")
	...
	for _, pos := range m.Positions.Positions {
		...
		line := fmt.Sprintf("$%s | Entry: %.5f | Mark: %.5f | PNL: %.1f%% %s",
			tokenName, pos.Size, pos.CurrentValue/100*pos.Size, pos.PnLPercent, status)
		posLines = append(posLines, lipgloss.NewStyle().Foreground(pnlColor).Render(line))
	}

	if len(posLines) == 0 {
		posLines = []string{lipgloss.NewStyle().Foreground(dimGreen).Render("No open positions")}
	}

	posContent := strings.Join(posLines, "\n")
	...
	totalPnL := m.Positions.TotalPnLPercent
	...
	// RAM bar (from Mode 4)
	ramBar := renderBar(ramPct, 10)

	statsContent := strings.Join([]string{
		statsStyle.Render(fmt.Sprintf("TOTAL ENTRIES: %d", entries)),
		statsStyle.Render(fmt.Sprintf("2X WINS: %d", wins)),
		statsStyle.Render(fmt.Sprintf("WIN RATE: %.1f%%", winRate)),
		"",
		dimStyle.Render(fmt.Sprintf("RAM: %s %s", ramBar, m.Header.MemUsage)),
		dimStyle.Render(fmt.Sprintf("TOTAL PNL: %+.1f%%", totalPnL)),
		dimStyle.Render(fmt.Sprintf("USD VALUE: $%.0f", usdEst)),
	}, "\n")
	...
	hotkeys := lipgloss.NewStyle().
		...
		Width(w).
		Align(lipgloss.Left).
		Render("HOTKEYS: [M] CYCLE MODE  |  [S] SELL ALL  |  [P] PAUSE/RESUME  |  [F9] CLEAR STATS  |  [Q] QUIT")

	// ─── FINAL ASSEMBLY ───
	rightPanel := lipgloss.JoinVertical(lipgloss.Left, posBox, statsBox)
	mainContent := lipgloss.JoinHorizontal(lipgloss.Top, signalBox, rightPanel)
...
// safeMintPrefix helper for FNEX mode
func safeMintPrefixFnex(mint string) string {
	if len(mint) >= 8 {
		return mint[:4] + "..." + mint[len(mint)-3:]
	}
	if len(mint) > 0 {
		return mint
	}
